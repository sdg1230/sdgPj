...
public class DesignerDao {
	...
	public ArrayList<String> selectAllDesignerSalon(Connection conn) {
		...
		String query = "select salon_name from salon";
		try {
			pstmt=conn.prepareStatement(query);
			rset=pstmt.executeQuery();
			while(rset.next()) {
	...
	public ArrayList<Designer> searchDesignerName(Connection conn, String keyword) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<Designer> list = new ArrayList<Designer>();
		String query = "select * from designer where designer_name like ?";
		try {
			pstmt=conn.prepareStatement(query);
	...
	public ArrayList<Designer> searchDesignerTwoName(Connection conn, String keyword, String salonName) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<Designer> list = new ArrayList<Designer>();
		String query = "select * from designer where salon_name=? and designer_name like ?";
		try {
			pstmt=conn.prepareStatement(query);
			pstmt.setString(1, salonName);
